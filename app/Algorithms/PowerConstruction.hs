...
transitionForPowerState :: (Ord a, Eq a) => State [a] -> [(State a, Char, State a)] -> Char 
                            -> (State [a], Char, State [a])
transitionForPowerState powerState transitions symbol = 
    (powerState, symbol, State ( uniqueSorted (quicksort 
                                              [name toState | (fromState, symbol', toState) <- transitions, 
                                                              elem (name fromState) (name powerState), 
                                                              symbol' == symbol])))

-- Create power set of transitions
powerSetOfTransitions :: (Ord a, Eq a) => [State [a]] -> [(State a, Char, State a)] -> [Char] 
                      -> [(State [a], Char, State [a])]
powerSetOfTransitions powerStates transitions alphabet = 
  [transitionForPowerState powerState transitions symbol | powerState <- powerStates, symbol <- alphabet]
